      ...
      >
        <div class="accordion-body">
          ...
          <template v-if="list.type === 'category'">
            <div
              class="form-check"
              v-for="item in categoryItems(list)"
              :key="item.id"
            >
              <input
                ...
                :name="list.id"
                :id="item.id"
                :checked="isChecked(item.id, list.id, list.type)"
                @change="updateEntityValue(list.id, item.id, list.type)"
              />
          ...
          <template v-else>
            // @todo there will be special fields, like picking a location,
            nationality etc
            ...
            <input
              ...
              class="form-control"
              :id="list.id"
              :value="entityValue(list.name, list.type)"
              @change="
                updateEntityValue(list.name, $event.target.value, list.type)
              "
              placeholder=""
            />
          </template>
...
export default {
  ...
  methods: {
    ...
    isChecked(category_item_id, category_id, type) {
      return (
        this.getEntityValue(
          this.$route.params.entityname,
          category_id,
          type
        ) === category_item_id
      );
